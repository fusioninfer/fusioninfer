...
// reconcilePodGroup creates or updates the PodGroup for gang scheduling
func (r *InferenceServiceReconciler) reconcilePodGroup(ctx context.Context, inferSvc *fusioninferiov1alpha1.InferenceService) error {
	...
	// Check if gang scheduling is needed
	needsGang := false
	for _, role := range inferSvc.Spec.Roles {
		if scheduling.NeedsGangScheduling(inferSvc, role) {
			needsGang = true
			break
		}
	}
	...
	if !needsGang {
		log.V(1).Info("No gang scheduling needed, skipping PodGroup creation")
		return nil
	}
...
// reconcileLWS creates or updates the LeaderWorkerSet for a role
func (r *InferenceServiceReconciler) reconcileLWS(ctx context.Context, inferSvc *fusioninferiov1alpha1.InferenceService, role fusioninferiov1alpha1.Role) error {
	...
	config := workload.LWSConfig{
		NeedsGangScheduling: scheduling.NeedsGangScheduling(inferSvc, role),
	}

	if config.NeedsGangScheduling {
		config.PodGroupName = scheduling.GeneratePodGroupName(inferSvc.Name)
		config.TaskName = string(role.ComponentType)
	}
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			log.Info("Creating LWS", "name", lws.Name, "role", role.Name)
			return r.Create(ctx, lws)
		}
		return err
	}
	...
	log.V(1).Info("Updating LWS", "name", lws.Name, "role", role.Name)
	return r.Update(ctx, existingLWS)
}
...
func (r *InferenceServiceReconciler) updateStatus(ctx context.Context, inferSvc *fusioninferiov1alpha1.InferenceService) error {
	...
	for _, role := range inferSvc.Spec.Roles {
		...
		lwsName := workload.GenerateLWSName(inferSvc.Name, role.Name)
		lws := &lwsv1.LeaderWorkerSet{}
		err := r.Get(ctx, types.NamespacedName{Name: lwsName, Namespace: inferSvc.Namespace}, lws)
		if err != nil {
			if apierrors.IsNotFound(err) {
				components[role.Name] = fusioninferiov1alpha1.ComponentStatus{
					DesiredReplicas: workload.GetReplicaCount(role),
					NodesPerReplica: workload.GetNodeCount(role),
		...
		status := fusioninferiov1alpha1.ComponentStatus{
			DesiredReplicas: desiredReplicas,
			ReadyReplicas:   lws.Status.ReadyReplicas,
			NodesPerReplica: nodesPerReplica,
			TotalPods:       desiredReplicas * nodesPerReplica,
			ReadyPods:       lws.Status.ReadyReplicas * nodesPerReplica,
			Phase:           determinePhase(lws, desiredReplicas),
			LastUpdateTime:  &metav1.Time{Time: metav1.Now().Time},
		}
...
// determinePhase determines the component phase based on LWS status
func determinePhase(lws *lwsv1.LeaderWorkerSet, desiredReplicas int32) fusioninferiov1alpha1.ComponentPhase {
	if lws.Status.ReadyReplicas == 0 {
		if lws.Status.Replicas == 0 {
			return fusioninferiov1alpha1.ComponentPhasePending
		}
		return fusioninferiov1alpha1.ComponentPhaseDeploying
	}

	if lws.Status.ReadyReplicas >= desiredReplicas {
		return fusioninferiov1alpha1.ComponentPhaseRunning
	}
